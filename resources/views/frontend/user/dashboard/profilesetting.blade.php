                    ...
                    <div class="col-md-4 p-4" style="border-right: 2px dashed #dee2e6;">
                        <div class="text-center">
                            <div class="mb-3">
                                ...
                                <img src="{{ asset('storage/'.$user->profile_image) }}" class="rounded-circle" style="width: 100px; height: 100px; object-fit: cover;">
                                @else
                                <i class="fas fa-user-circle text-muted" style="font-size: 4rem;"></i>
                                @endif
                            </div>
                            <h6 class="mb-1">{{ $user->username ?? 'Username' }}</h6>
                            <p class="text-muted small mb-3">{{ $user->username ?? 'Not set' }}</p>
                            ...
                            <p class="small mb-3">{{ $user->email }}</p>
                            ...
                            <p class="small mb-3">{{ $user->phone ?? 'Not provided' }}</p>
                            ...
                            <p class="small mb-3">{{ $user->country ?? 'Not provided' }}</p>
                            ...
                            <p class="small mb-3">{{ $user->address ?? 'Not provided' }}</p>
                        </div>
                    </div>
                        ...
                        <form method="POST" action="{{ route('user.profile.update') }}" enctype="multipart/form-data">
                            ...
                            <div class="text-center mb-4">
                                <div class="border border-2 border-dashed rounded position-relative" style="height: 150px; background-color: #f8f9fa; border-color: rgb(251,109,2) !important; cursor: pointer; overflow: hidden;" onclick="document.getElementById('imageInput').click();">
                                    @if($user->profile_image)
                                    <img id="previewImage" src="{{ asset('storage/'.$user->profile_image) }}" class="position-absolute top-0 start-0" style="width: 100%; height: 100%; object-fit: cover;" />
                                    @else
                                    ...
                                    @endif
                                    <input type="file" name="profile_image" id="imageInput" accept="image/jpeg,image/jpg,image/png" style="display: none;" onchange="previewFile(this)" />
                                </div>
                                <div class="mt-2">
